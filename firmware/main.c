...
volatile uint8_t relay_flag;

//TODO: wrap asm_tx and asm_rx in ATOMIC to prevent timing issues.
...
//TODO: replace with sprintf ( char * str, const char * format, ... )
...
ISR(PCINT1_vect)
{
	...
	*/
	...
	PCIFR = (1 << PCIF1); //What happens in the interrupt service routine, stays in the interrupt service routine.	
}
...
int main( void ) {
	...
	DDRC = (0 << DDC0) | (1 << DDC1) | (1 << DDC5) | (1 << DDC2) | (1 << DDC3) | (0 << DDC4); 
	PORTC = 0x00;
	...
	PCMSK1 |= (1 << PCINT12); //Mask to recieve just PC4's Interrupt
